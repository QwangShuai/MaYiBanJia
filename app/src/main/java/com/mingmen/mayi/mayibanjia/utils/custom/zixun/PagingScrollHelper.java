...
public class PagingScrollHelper {
    ...
    private QuanBuCaiPinLeiOneAdapter adapter = null;
    ...
    public void setAdapter(RecyclerView.Adapter adapter,int type) {
        if(type==0){
            ...
            this.adapter = (QuanBuCaiPinLeiOneAdapter) adapter;
            update2();
        }
    }
    ...
    // 更新页码指示器和相关数据
    private void update() {
        int temp = myAdapter.getPageCount();
        if (temp != totalPage) {
    ...
    private void update2() {
        int temp = adapter.getPageCount();
        Log.e("update2: ","yemian" + temp );
        if (temp != totalPage) {
            mIndicatorView.initIndicator(temp);
            if (temp < totalPage && index == totalPage) {
                index = temp;
